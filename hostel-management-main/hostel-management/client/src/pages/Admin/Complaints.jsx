  ...
  >
    ...
    <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
      <div className="flex items-center space-x-1">
        ...
        <span>{complaint.student?.name} ({complaint.student?.studentId})</span>
      </div>
    </div>
    ...
    <div className="flex items-center justify-between text-sm text-gray-500">
      ...
      {complaint.comments && complaint.comments.length > 0 && (
        <div className="flex items-center space-x-1">
          ...
          <span>{complaint.comments.length} comment{complaint.comments.length !== 1 ? 's' : ''}</span>
        </div>
      )}
    </div>
  </div>
...
const ComplaintModal = ({ isOpen, onClose, complaint, onUpdateStatus, onAddComment }) => {
  ...
  const handleStatusUpdate = async (status, resolution = '') => {
    setIsUpdatingStatus(true);
    try {
      await onUpdateStatus(complaint._id, { status, resolution });
    } finally {
                ...
                {(complaint.status === 'Open' || complaint.status === 'In Progress') && (
                  <button
                    onClick={() => {
                      ...
                      if (resolution) {
                        handleStatusUpdate('Resolved', resolution);
                      }
                    }}
                    disabled={isUpdatingStatus}
                      ...
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-medium text-gray-900">
                            {comment.author?.name} ({comment.authorModel})
                          </span>
...
const AdminComplaints = () => {
  ...
  const updateMutation = useMutation(
    ({ id, data }) => api.put(`/complaints/${id}`, data),
    {
      onSuccess: () => {
        ...
        if (selectedComplaint) {
          setSelectedComplaint({ ...selectedComplaint, ...updateMutation.variables.data });
        }
      },
      onError: (error) => {
  ...
  const handleUpdateStatus = async (complaintId, data) => {
    return updateMutation.mutateAsync({ id: complaintId, data });
  };
  ...
  const filteredComplaints = complaints?.filter(complaint => {
    ...
    return (
      ...
      complaint.student?.name.toLowerCase().includes(searchTerm) ||
      complaint.student?.studentId.toLowerCase().includes(searchTerm)
    );
  });
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {filteredComplaints && filteredComplaints.length > 0 ? (
        ...
        <div className="text-center py-12">
          <MessageSquareMore className="w-16 h-16 text-gray-300 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-gray-900 mb-2">
          ...
          <p className="text-gray-600">
            {filters.search || filters.status || filters.category || filters.priority
              ...
              : 'All complaints will appear here when students submit them'
            }
          </p>
        </div>
      )}
